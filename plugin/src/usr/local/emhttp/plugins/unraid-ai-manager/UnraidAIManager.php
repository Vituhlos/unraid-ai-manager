...
function cfg_default($cfg, $key, $fallback) {
  return array_key_exists($key, $cfg) && $cfg[$key] !== "" ? $cfg[$key] : $fallback;
}

function shell_quote_cfg($value) {
  return '"' . str_replace(['\\', '"', '$', '`'], ['\\\\', '\\"', '\\$', '\\`'], $value) . '"';
}

function write_config($path, $cfg) {
  $dir = dirname($path);
  if (!is_dir($dir)) {
    mkdir($dir, 0700, true);
  }
  ...
  foreach ($cfg as $key => $value) {
    $lines[] = $key . "=" . shell_quote_cfg($value);
  }
...
foreach ($defaults as $key => $value) {
  $cfg[$key] = cfg_default($cfg, $key, $value);
}
...
$status = trim(shell_exec("$rc status 2>&1"));
$hasKey = ($cfg["API_KEY"] ?? "") !== "";
...
<div class="unraid-ai-manager">
  ...
  <?php if ($message): ?>
    <blockquote style="white-space: pre-wrap;"><?= h($message) ?></blockquote>
  <?php endif; ?>
  <?php if ($error): ?>
    <blockquote style="white-space: pre-wrap; color: #f66;"><?= h($error) ?></blockquote>
  <?php endif; ?>
  ...
  <form method="post">
    <table class="share_status">
      <tr><td>Listen</td><td><input type="text" name="LISTEN" value="<?= h($cfg["LISTEN"]) ?>" style="width: 420px;"></td></tr>
      <tr><td>Templates</td><td><input type="text" name="TEMPLATES_DIR" value="<?= h($cfg["TEMPLATES_DIR"]) ?>" style="width: 650px;"></td></tr>
      <tr><td>Backup dir</td><td><input type="text" name="BACKUP_DIR" value="<?= h($cfg["BACKUP_DIR"]) ?>" style="width: 650px;"></td></tr>
      <tr><td>Audit dir</td><td><input type="text" name="AUDIT_DIR" value="<?= h($cfg["AUDIT_DIR"]) ?>" style="width: 650px;"></td></tr>
      <tr><td>Plans dir</td><td><input type="text" name="PLANS_DIR" value="<?= h($cfg["PLANS_DIR"]) ?>" style="width: 650px;"></td></tr>
      <tr><td>Approvals dir</td><td><input type="text" name="APPROVALS_DIR" value="<?= h($cfg["APPROVALS_DIR"]) ?>" style="width: 650px;"></td></tr>
      <tr><td>Docker socket</td><td><input type="text" name="DOCKER_SOCKET" value="<?= h($cfg["DOCKER_SOCKET"]) ?>" style="width: 650px;"></td></tr>
      <tr><td>Docker host</td><td><input type="text" name="DOCKER_HOST" value="<?= h($cfg["DOCKER_HOST"]) ?>" style="width: 650px;"></td></tr>
      <tr><td>Default local host/IP</td><td><input type="text" name="LOCAL_HOST" value="<?= h($cfg["LOCAL_HOST"]) ?>" style="width: 420px;" placeholder="192.0.2.10"></td></tr>
      <tr><td>API key</td><td><code><?= h($keyPreview) ?></code> <button type="submit" name="action" value="generate_key">Generate new key</button></td></tr>
...
/usr/local/bin/unraid-ai-manager approve-plan \
  --plan <?= h($cfg["PLANS_DIR"]) ?>/PLAN.json \
  --approvals-dir <?= h($cfg["APPROVALS_DIR"]) ?> \
  --purpose amud \
  --ttl 15m
  </pre>
  ...
  <pre>ssh -L 37231:127.0.0.1:37231 root@<?= h($_SERVER['SERVER_NAME'] ?? 'unraid') ?></pre>
  <p>
    MCP server environment on PC:
  </p>
  <pre>
UNRAID_AI_HELPER_URL=http://127.0.0.1:37231
UNRAID_AI_API_KEY=&lt;the API key from <?= h($config) ?>&gt;
  </pre>
</div>
