...
- Added a read-only dashboard integration readiness plan (`dashboard-integrations`) that combines dashboard candidates with discovered `secret_ref` values and reports `ready`, `missing-secret` or `unsupported` status.
